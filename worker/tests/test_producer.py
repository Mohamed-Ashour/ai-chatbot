...
class TestProducer:
    ...
    async def test_add_to_stream_exception_handling(self, producer, capfd):
        ...
        producer.redis_client.xadd = AsyncMock(side_effect=Exception("Redis connection error"))
    ...
    async def test_expire_key_not_exists(self, producer):
        ...
        producer.redis_client.expire = AsyncMock(return_value=False)  # Key doesn't exist
    ...
    async def test_integration_add_and_expire(self, producer):
        ...
        producer.redis_client.expire.assert_called_once_with(stream_channel, expire_seconds)
