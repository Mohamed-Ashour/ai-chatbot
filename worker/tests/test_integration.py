...
class TestWorkerIntegration:
    ...
    async def test_end_to_end_message_processing(self):
        ...
        chat_history = {
            "messages": [
                {"id": "msg1", "msg": "Previous message", "timestamp": "2024-01-01T10:00:00", "source": "user"},
                {"id": "msg2", "msg": "Previous response", "timestamp": "2024-01-01T10:00:01", "source": "assistant"}
            ]
        }
        ...
        with patch('src.model.gpt.AsyncGroq') as mock_groq:
            ...
            mock_gpt_response.choices[0].message.content = "I'm doing well, thank you for asking!"
            mock_gpt_response.model_dump.return_value = {"test": "response"}
            
            mock_groq_client = AsyncMock()
            mock_groq_client.chat.completions.create = AsyncMock(return_value=mock_gpt_response)
            ...
            with patch('redis.asyncio.from_url', return_value=mock_redis_client):
                ...
                mock_redis_client.json().get.assert_called_once_with("test_token", cache.redis_client.json().get.call_args[0][1])
                ...
                mock_redis_client.expire.assert_called_once_with("response_channel_test_token", 3600)
            ...
            with patch.object(Redis, '__init__', track_redis_init):
                with patch.object(Cache, '__init__', track_cache_init):
                    with patch.object(GPT, '__init__', track_gpt_init):
                        with patch.object(StreamConsumer, '__init__', track_consumer_init):
                            with patch.object(Producer, '__init__', track_producer_init):
    ...
    async def test_concurrent_message_processing(self):
        ...
        with patch('redis.asyncio.from_url', return_value=mock_redis_client):
            with patch('src.model.gpt.AsyncGroq') as mock_groq:
                ...
                mock_groq_client = AsyncMock()
                mock_groq_client.chat.completions.create = AsyncMock(side_effect=[mock_response1, mock_response2])
                mock_groq.return_value = mock_groq_client
