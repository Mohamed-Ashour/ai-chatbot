...
class TestStreamConsumer:
    ...
    @pytest.mark.asyncio
    async def test_consume_stream_success(
        self, stream_consumer, sample_stream_response
    ):
        """Test successful stream consumption."""
        stream_consumer.redis_client.xread = AsyncMock(
            return_value=sample_stream_response
        )
        ...
        stream_consumer.redis_client.xread.assert_called_once_with(
            streams={"message_channel": "0-0"}, count=1, block=0
        )
    ...
    async def test_consume_stream_multiple_count(self, stream_consumer):
        ...
        stream_consumer.redis_client.xread = AsyncMock(
            return_value=multiple_messages_response
        )

        result = await stream_consumer.consume_stream(
            stream_channel="message_channel", count=3, block=0
        )

        stream_consumer.redis_client.xread.assert_called_once_with(
            streams={"message_channel": "0-0"}, count=3, block=0
        )
    ...
    async def test_consume_stream_blocking(self, stream_consumer):
        ...
        await stream_consumer.consume_stream(
            stream_channel="message_channel", count=1, block=5000  # Block for 5 seconds
        )

        stream_consumer.redis_client.xread.assert_called_once_with(
            streams={"message_channel": "0-0"}, count=1, block=5000
        )
    ...
    async def test_consume_stream_different_channel(self, stream_consumer):
        ...
        await stream_consumer.consume_stream(
            stream_channel="custom_channel", count=1, block=0
        )

        stream_consumer.redis_client.xread.assert_called_once_with(
            streams={"custom_channel": "0-0"}, count=1, block=0
        )
    ...
    async def test_delete_message_success(self, stream_consumer):
        ...
        stream_consumer.redis_client.xdel = AsyncMock(
            return_value=1
        )  # 1 message deleted

        await stream_consumer.delete_message(
            stream_channel="message_channel", message_id="1234567890123-0"
        )

        stream_consumer.redis_client.xdel.assert_called_once_with(
            "message_channel", "1234567890123-0"
        )
    ...
    async def test_delete_message_not_found(self, stream_consumer):
        ...
        await stream_consumer.delete_message(
            stream_channel="message_channel", message_id="nonexistent-message-id"
        )

        stream_consumer.redis_client.xdel.assert_called_once_with(
            "message_channel", "nonexistent-message-id"
        )
    ...
    async def test_consume_stream_response_format(self, stream_consumer):
        ...
        result = await stream_consumer.consume_stream(
            stream_channel="message_channel", count=2, block=0
        )
