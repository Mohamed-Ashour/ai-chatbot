...
class TestCache:
    ...
    async def test_get_chat_history_success(
        self, cache, sample_chat_history, sample_token
    ):
        ...
        cache.redis_client.json().get.assert_called_once_with(
            sample_token, Path.root_path()
        )
    ...
    async def test_get_chat_history_with_limit(self, cache, sample_token):
        ...
        assert (
            result["messages"][0]["id"] == "msg10"
        )  # Last 5 messages: 10, 11, 12, 13, 14
    ...
    @pytest.mark.asyncio
    async def test_get_chat_history_limit_larger_than_messages(
        self, cache, sample_token
    ):
    ...
    async def test_add_messages_to_cache_exception_handling(self, cache, sample_token):
        ...
        cache.redis_client.json().arrappend = AsyncMock(
            side_effect=Exception("Redis error")
        )

        # Should propagate the exception
        with pytest.raises(Exception, match="Redis error"):
            await cache.add_messages_to_cache(
                token=sample_token, messages=[message_data]
            )

    @pytest.mark.asyncio
    async def test_get_chat_history_zero_limit(
        self, cache, sample_chat_history, sample_token
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_chat_history_negative_limit(
        self, cache, sample_chat_history, sample_token
    ):
