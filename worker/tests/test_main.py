...
class TestProcessMessage:
    ...
    async def test_process_message_success(
        self, cache, mock_gpt, producer, sample_token, sample_chat_history, capfd
    ):
        ...
        gpt_response = Message(
            msg="I'm doing well, thank you!", source=SourceEnum.assistant
        )
        ...
        producer.expire.assert_called_once_with(
            f"response_channel_{sample_token}", 3600
        )
    ...
    async def test_process_message_expired_session(
        self, cache, mock_gpt, producer, sample_token, capfd
    ):
    ...
    @pytest.mark.asyncio
    async def test_process_message_empty_chat_history(
        self, cache, mock_gpt, producer, sample_token
    ):
        ...
        gpt_response = Message(
            msg="Hello! How can I help?", source=SourceEnum.assistant
        )
        mock_gpt.query = AsyncMock(return_value=gpt_response)
    ...
    @pytest.mark.asyncio
    async def test_process_message_gpt_error(
        self, cache, mock_gpt, producer, sample_token, sample_chat_history
    ):
    ...
    @pytest.mark.asyncio
    async def test_process_message_cache_error(
        self, cache, mock_gpt, producer, sample_token
    ):
    ...
    @pytest.mark.asyncio
    async def test_process_message_producer_error(
        self, cache, mock_gpt, producer, sample_token, sample_chat_history
    ):
    ...
    @pytest.mark.asyncio
    async def test_process_message_creates_correct_user_message(
        self, cache, mock_gpt, producer, sample_token, sample_chat_history
    ):
...
class TestMain:
    ...
    async def test_main_initialization(
        ...
        mock_redis_instance.create_connection = AsyncMock(
        ...
        mock_consumer.consume_stream = AsyncMock(
            return_value=[]
    ...
    async def test_main_message_processing_loop(
        ...
        mock_consumer.consume_stream = AsyncMock(
            side_effect=[stream_response, KeyboardInterrupt()]
        )
        ...
        mock_consumer.delete_message.assert_called_once_with(
            stream_channel="message_channel", message_id=b"1234567890123-0"
        )

    @patch("main.Redis")
    @pytest.mark.asyncio
    async def test_main_redis_connection_cleanup(self, mock_redis_class, capfd):
    ...
    @patch("main.Redis")
    @pytest.mark.asyncio
    async def test_main_redis_cleanup_error(self, mock_redis_class, capfd):
    ...
    @patch("main.Redis")
    @pytest.mark.asyncio
    async def test_main_general_exception_handling(self, mock_redis_class, capfd):
