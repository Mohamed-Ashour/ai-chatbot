...
export default function ChatPage() {
  ...
  // Close modal only when successfully connected
  const showWelcome = isModalOpen && connectionStatus !== 'connected'
  ...
  useEffect(() => {
    if (connectionStatus === 'connected' && isModalOpen) {
      setIsModalOpen(false)
    }
  }, [connectionStatus, isModalOpen])
  ...
  return (
    <div className="flex flex-col h-screen bg-gradient-to-br from-indigo-50 via-white to-cyan-50 overflow-hidden">
      ...
      >
        {isConnected || (hasValidSession && userName) ? (
          <>
