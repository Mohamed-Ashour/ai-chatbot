...
class TestWebSocketAuthentication:
    ...
    def test_websocket_connection_invalid_token(self, client):
        ...
        with pytest.raises(Exception):
            with client.websocket_connect("/chat?token=invalid-token") as websocket:
                pass
    ...
    def test_websocket_connection_empty_token(self, client):
        ...
        with pytest.raises(Exception):
            with client.websocket_connect("/chat?token=") as websocket:
                pass
    
    @pytest.mark.websocket
    def test_websocket_connection_valid_token_simulation(self, mock_redis_config, fake_redis, sample_chat_session):
        ...
        # 1. Use a different testing framework (like pytest-asyncio with actual WebSocket clients)
        # 2. Test the WebSocket endpoint in integration tests with a running server
        # 3. Test individual components (token validation, message processing) separately
...
class TestWebSocketCommunication:
    ...
    @pytest.mark.websocket
    async def test_websocket_message_flow(self, mock_redis_config, fake_redis, sample_chat_session):
        ...
        with patch('src.routes.chat.Producer') as mock_producer_class, \
             patch('src.routes.chat.StreamConsumer') as mock_consumer_class, \
...
class TestWebSocketUtilities:
    ...
    async def test_get_token_invalid(self, mock_redis_config, fake_redis):
        ...
        assert "Session not authenticated or expired token" in str(exc_info.value.reason)
