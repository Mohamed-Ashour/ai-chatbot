...
class TestRedisConfiguration:
    ...
    def test_redis_config_initialization(self):
        ...
        assert (
            "redis://default:testpassword@localhost:6379" in redis_config.connection_url
        )
...
class TestRedisProducer:
    ...
    async def test_add_to_stream_success(self, fake_redis):
        ...
        message_id = await producer.add_to_stream(
            data=test_data, stream_channel=stream_name
        )
    ...
    async def test_producer_error_handling(self, fake_redis):
        ...
        result = await producer.add_to_stream(
            data={"test": "data"}, stream_channel="test_stream"
        )
...
class TestRedisStreamConsumer:
    ...
    async def test_consume_stream_success(self, fake_redis):
        ...
        result = await consumer.consume_stream(
            stream_channel=stream_name, count=1, block=0
        )
        ...
        if len(result) > 0:
            ...
            assert (
                stream_name_result.decode() == stream_name
                if isinstance(stream_name_result, bytes)
                else stream_name_result == stream_name
            )
    ...
    async def test_consume_stream_empty(self, fake_redis):
        ...
        result = await consumer.consume_stream(
            stream_channel="non_existent_stream", count=1, block=0
        )
    ...
    async def test_consume_stream_with_count(self, fake_redis):
        ...
        for i in range(5):
            msg_id = await producer.add_to_stream(
                data={f"token{i}": f"Message {i}"}, stream_channel=stream_name
            )
            message_ids.append(msg_id)

        # Consume with specific count
        result = await consumer.consume_stream(
            stream_channel=stream_name, count=3, block=0
        )
    ...
    async def test_stream_operations_integration(self, fake_redis):
        ...
        result = await consumer.consume_stream(
            stream_channel=stream_name, count=10, block=0  # Request more than available
        )
...
class TestRedisSessionManagement:
    ...
    async def test_session_creation_and_retrieval(
        self, fake_redis, sample_chat_session
    ):
