...
# Import the main application (imports must be after sys.path modification)
from main import api  # noqa: E402
from src.redis.config import Redis  # noqa: E402
from src.schema.chat import Chat, Message, SourceEnum  # noqa: E402
        ...
        async def xread(self, streams, count=None, block=None):
            ...
            for stream_name in streams:
                if stream_name in self._streams and self._streams[stream_name]:
                    messages = (
                        self._streams[stream_name][:count]
                        if count
                        else self._streams[stream_name]
                    )
                    if messages:
                        result.append(
                            (
        ...
        async def xdel(self, stream, *message_ids):
            if stream in self._streams:
                ...
                self._streams[stream] = [
                    (msg_id, fields)
                    for msg_id, fields in self._streams[stream]
...
def sample_chat_session():
    ...
    return Chat(
        ...
        name="Test User",
        messages=[Message(id="msg-1", msg="Hello, world!", source=SourceEnum.user)],
    )
...
def sample_message():
    ...
    return Message(id="test-msg-id", msg="Test message content", source=SourceEnum.user)
...
def mock_consumer():
    ...
    mock.consume_stream.return_value = [
        ("response_channel_test", [("msg-id-1", {b"message": b"Test AI response"})])
    ]
