...
async def token_generator(name: str = Form()):
    ...
    result = await redis_client.json().set(
        str(token), Path.root_path(), chat_session.model_dump()
    )
...
async def chat_history(token: str):
    redis_client = await redis.create_connection()
    ...
    try:
        while True:
            ...
            await producer.add_to_stream(
                data=stream_data, stream_channel=message_channel
            )
            response = await consumer.consume_stream(
                stream_channel=response_channel, count=1, block=0
            )
            for stream, messages in response:
                for message_id, message_data in messages:
                    ...
                    await consumer.delete_message(
                        stream_channel=response_channel, message_id=message_id
                    )
